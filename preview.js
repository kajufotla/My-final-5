...
export const renderList = (textId, listId, wrapId) => {
  ...
  // Upgrade: Also hide/show the parent premium-card so we don't get empty white boxes in the new design
  const premiumCard = wrap.closest('.premium-card');
  
  if (text.trim()) { 
    wrap.style.display = 'flex'; 
    if (premiumCard && premiumCard.id !== wrapId) premiumCard.style.display = 'flex';
    list.innerHTML = text.replace(/\n/g, '<br>'); 
  } 
  else {
    wrap.style.display = 'none';
    if (premiumCard && premiumCard.id !== wrapId) premiumCard.style.display = 'none';
  }
};

export const updatePreview = (cache, state, sanitizeHTML) => {
  if (!cache || !state) return; // Defensive check for robust performance
  ...
  if(cache.paymentArchType?.value === 'bank') {
    ...
    if(payUrl && cache.prevPayUrl) {
      ...
      cache.prevPayUrl.textContent = "Click here to Pay ↗"; 
  ...
  if(cache.prevItemsBody) {
      // Enterprise Upgrade: Using DocumentFragment to prevent multiple DOM repaints (extremely fast rendering)
      ...
      state.items.forEach((item, index) => {
        ...
        tr.innerHTML = `<td style="text-align:center;">${index + 1}</td>
                        <td>${sanitizeHTML(item.desc)}</td>
                        <td style="text-align:center;">${q||''}</td>
                        <td style="text-align:right;">${formatMoney(p, cache.currencySelect?.value)}</td>
                        <td style="text-align:right;">${formatMoney(t, cache.currencySelect?.value)}</td>`;
        fragment.appendChild(tr);
      });

      cache.prevItemsBody.innerHTML = '';
      cache.prevItemsBody.appendChild(fragment);
      ...
      // Upgrade: Pushing calculated values to the DOM before showing the row
      if(cache.prevDiscount) cache.prevDiscount.textContent = `-${formatMoney(d, cache.currencySelect?.value)}`;
      if(cache.rowDiscount) cache.rowDiscount.style.display = d > 0 ? 'flex' : 'none';
      
      if(cache.prevTax) cache.prevTax.textContent = formatMoney(taxAmt, cache.currencySelect?.value);
      if(cache.rowTax) cache.rowTax.style.display = taxAmt > 0 ? 'flex' : 'none';
      
      if(cache.prevShipping) cache.prevShipping.textContent = formatMoney(s, cache.currencySelect?.value);
  ...
  } else if (document.getElementById('prevDueDate')) {
      document.getElementById('prevDueDate').style.display = 'none';
  }
};
